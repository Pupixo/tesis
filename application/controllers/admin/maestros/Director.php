...
class Director extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->modulo            = 'Modulo:: Director';  /* DESCRIPCION ( MODULO :: NRO DE MENU  )                           */
        $this->tituloPrincipal   = 'Director';   /* NOMBRE PRINCIPAL 1                                               */
        $this->tituloSecundario1 = 'Listado de Director';     /* NOMBRE SECUNDARIO 1, PUEDE HABER HASTA 5 VARIABLES SECUNDARIAS   */
        $this->formPrincipal     = 'formulario_director';   /* NOMBRE DEL FORMULARIO                                            */
        $this->opcion            = 'Director';        /* NOMBRE DE LA CLASE                                               */
        $this->url               = 'admin/maestros/';     /* URL DE LA PAGINA ACTUAL                                          */
        $this->abrev             = 'director';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */
        $this->url_carpeta       = 'admin/maestros/director/';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */

        $this->load->model('Contenedor_Model','contenedor');
        $this->load->model('Model_Director','main_model');
    ...
    function cargar_tabla_Director()
    {
        if ($this->session->userdata('usuario')) {
            date_default_timezone_set('America/Lima');
            $fechaActual = date('Y-m-d H:i:s');

            $accion = 'LISTADO_DIRECTOR';
            $parametros = array($accion,'', '', '', '', '','');
            
            // print_r($parametros);
            // exit();
     
            $data = $this->main_model->procedureCrudDirector($parametros);
                    $texto = '{"data":[';
                    $fila =0;
                    foreach ($data as $row) {
                        ...
                        $botones = "<center style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;'>"   
                                        ...
                                        . "<div class='btn-group' role='group'>"
                                            ...
                                            . "<div  style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;' class='dropdown-menu rueda-accion color-0' aria-labelledby='btnGroupDrop".$this->opcion."'>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick=fn_AbrirModal('A',". $row['id_director'] . ",".$fila.",'Insert_Update_".$this->opcion."') class='dropdown-item delay-toogle btn-table-modal' title='Editar ".$this->abrev ."'  >"  	
                                                ."<span>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick='Eliminar_".$this->opcion."(".$row['id_director'].")' id='delete' role='button' class='dropdown-item delay-toogle btn-table-modal'  title='Eliminar ".$this->abrev ."' >"
                                                ."<span>"
                        ...
                        $texto .= '{"ID_DIRECTOR":' . json_encode($row['id_director']) . ','
                                . '"NOM_DIRECTOR":' . json_encode($row['nom_director']) . ','
                                . '"ID_FACULTAD":' . json_encode($row['id_facultad']) . ','   
                                . '"ID_DEPART_UNI":' . json_encode($row['id_depart_uni']) . ',' 
    ...
    //---------------------------------------------------------------------------
    public function Insert_Director(){
        if ($this->session->userdata('usuario')) {
            $user_reg= $_SESSION['usuario'][0]['id_usuario'];

            $nom_director               = $this->input->post("nom_director");            
            $id_status              = $this->input->post("cbx_basicos_id_status");



            $accion = 'INSERTAR_DIRECTOR';
            $parametros = array(
                $accion,
                '',
                $nom_director,
           ...
           //$respuesta=
            $this->main_model->procedureCrudDirector($parametros);
            // echo "<pre>";
    ...
    public function Update_Director(){
        if ($this->session->userdata('usuario')) {
            ...
            $id_director              = $this->input->post("id_director");
            $nom_director               = $this->input->post("nom_director");            
            $id_status              = $this->input->post("cbx_basicos_id_status");
            ...
            $respuesta= $this->main_model->procedureCrudDirector($parametros);
            // echo "<pre>";
