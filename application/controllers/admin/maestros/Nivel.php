...
class Nivel extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->modulo            = 'Modulo:: Nivel';  /* DESCRIPCION ( MODULO :: NRO DE MENU  )                           */
        $this->tituloPrincipal   = 'Nivel';   /* NOMBRE PRINCIPAL 1                                               */
        $this->tituloSecundario1 = 'Listado de Nivel';     /* NOMBRE SECUNDARIO 1, PUEDE HABER HASTA 5 VARIABLES SECUNDARIAS   */
        $this->formPrincipal     = 'formulario_nivel';   /* NOMBRE DEL FORMULARIO                                            */
        $this->opcion            = 'Nivel';        /* NOMBRE DE LA CLASE                                               */
        $this->url               = 'admin/maestros/';     /* URL DE LA PAGINA ACTUAL                                          */
        $this->abrev             = 'nivel';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */
        $this->url_carpeta       = 'admin/maestros/nivel/';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */

        $this->load->model('Contenedor_Model','contenedor');
        $this->load->model('Model_Nivel','main_model');
    ...
    function cargar_tabla_Nivel()
    {
        if ($this->session->userdata('usuario')) {
            date_default_timezone_set('America/Lima');
            $fechaActual = date('Y-m-d H:i:s');

            $accion = 'LISTADO_NIVEL';
            $parametros = array($accion,'', '', '', '');
            
            // print_r($parametros);
            // exit();
     
            $data = $this->main_model->procedureCrudNivel($parametros);
                    $texto = '{"data":[';
                    $fila =0;
                    foreach ($data as $row) {
                        ...
                        $botones = "<center style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;'>"   
                                        ...
                                        . "<div class='btn-group' role='group'>"
                                            ...
                                            . "<div  style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;' class='dropdown-menu rueda-accion color-0' aria-labelledby='btnGroupDrop".$this->opcion."'>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick=fn_AbrirModal('A',". $row['id_nivel'] . ",".$fila.",'Insert_Update_".$this->opcion."') class='dropdown-item delay-toogle btn-table-modal' title='Editar ".$this->abrev ."'  >"  	
                                                ."<span>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick='Eliminar_".$this->opcion."(".$row['id_nivel'].")' id='delete' role='button' class='dropdown-item delay-toogle btn-table-modal'  title='Eliminar ".$this->abrev ."' >"
                                                ."<span>"
                        ...
                        $texto .= '{"ID_NIVEL":' . json_encode($row['id_nivel']) . ','
                                . '"NOM_NIVEL":' . json_encode($row['nom_nivel']) . ','
    ...
    //---------------------------------------------------------------------------
    public function Insert_Nivel(){
        if ($this->session->userdata('usuario')) {
            $user_reg= $_SESSION['usuario'][0]['id_usuario'];

            $nom_nivel               = $this->input->post("nom_nivel");            
           ...
           //$respuesta=
            $this->main_model->procedureCrudNivel($parametros);
            // echo "<pre>";
    ...
    public function Update_Nivel(){
        if ($this->session->userdata('usuario')) {
            ...
            $respuesta= $this->main_model->procedureCrudNivel($parametros);
            // echo "<pre>";
