...
class Docente extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->modulo            = 'Modulo:: Docente';  /* DESCRIPCION ( MODULO :: NRO DE MENU  )                           */
        $this->tituloPrincipal   = 'Docente';   /* NOMBRE PRINCIPAL 1                                               */
        $this->tituloSecundario1 = 'Listado de Docente';     /* NOMBRE SECUNDARIO 1, PUEDE HABER HASTA 5 VARIABLES SECUNDARIAS   */
        $this->formPrincipal     = 'formulario_docente';   /* NOMBRE DEL FORMULARIO                                            */
        $this->opcion            = 'Docente';        /* NOMBRE DE LA CLASE                                               */
        $this->url               = 'admin/maestros/';     /* URL DE LA PAGINA ACTUAL                                          */
        $this->abrev             = 'docente';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */
        $this->url_carpeta       = 'admin/maestros/docente/';        /* NOMBRE DE LA ABREVIATURA QUE SE INDENTIFICARA EN "footer" y js   */

        $this->load->model('Contenedor_Model','contenedor');
        $this->load->model('Model_Docente','main_model');
    ...
    function cargar_tabla_Docente()
    {
        if ($this->session->userdata('usuario')) {
            date_default_timezone_set('America/Lima');
            $fechaActual = date('Y-m-d H:i:s');

            $accion = 'LISTADO_DOCENTE';
            $parametros = array($accion,'', '', '', '', '','');
            
            // print_r($parametros);
            // exit();
     
            $data = $this->main_model->procedureCrudDocente($parametros);
                    $texto = '{"data":[';
                    $fila =0;
                    foreach ($data as $row) {
                        ...
                        $botones = "<center style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;'>"   
                                        ...
                                        . "<div class='btn-group' role='group'>"
                                            ...
                                            . "<div  style='background:rgb(215, 211, 211) none repeat scroll 0% 0%;border-radius:20px;' class='dropdown-menu rueda-accion color-0' aria-labelledby='btnGroupDrop".$this->opcion."'>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick=fn_AbrirModal('A',". $row['id_docente'] . ",".$fila.",'Insert_Update_".$this->opcion."') class='dropdown-item delay-toogle btn-table-modal' title='Editar ".$this->abrev ."'  >"  	
                                                ."<span>"
                                                ...
                                                ."<a style='cursor: pointer;' onclick='Eliminar_".$this->opcion."(".$row['id_docente'].")' id='delete' role='button' class='dropdown-item delay-toogle btn-table-modal'  title='Eliminar ".$this->abrev ."' >"
                                                ."<span>"
                        ...
                        $texto .= '{"ID_DOCENTE":' . json_encode($row['id_docente']) . ','
                                . '"NOM_DOCENTE":' . json_encode($row['nom_docente']) . ','
    ...
    public function Insert_Docente(){
        if ($this->session->userdata('usuario')) {
            ...
            $nom_docente               = $this->input->post("nom_docente");            
            $id_status              = $this->input->post("cbx_basicos_id_status");

            $accion = 'INSERTAR_DOCENTE';
            $parametros = array(
                $accion,
                '',
                $nom_docente,
           ...
           //$respuesta=
            $this->main_model->procedureCrudDocente($parametros);
            // echo "<pre>";
    ...
    public function Update_Docente(){
        if ($this->session->userdata('usuario')) {
            ...
            $respuesta= $this->main_model->procedureCrudDocente($parametros);
            // echo "<pre>";
